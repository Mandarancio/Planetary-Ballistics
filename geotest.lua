function PointWithinShape(shape, tx, ty)
	if #shape == 0 then
		return false
	elseif #shape == 2 then
		return shape[1] == tx and shape[2] == ty
	elseif #shape == 4 then
...
function BoundingBox(box, tx, ty)
	return	(box[3] >= tx and box[4] >= ty)
		and (box[1] <= tx and box[2] <= ty)
		or  (box[1] >= tx and box[4] >= ty)
		and (box[3] <= tx and box[2] <= ty)
end

function colinear(line, x, y, e)
	e = e or 0.1
	m = (line[4] - line[2]) / (line[3] - line[1])
	local function f(x) return line[2] + m*(x - line[1]) end
...
function CrossingsMultiplyTest(pgon, tx, ty)
	...
	local numverts = #pgon/2
	vtx0 = {x=pgon[numverts*2-1],y=pgon[numverts*2]}
	vtx1 = {x=pgon[1],y=pgon[2]}
