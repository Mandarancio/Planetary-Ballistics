...
function Game:generate_system(N, M, sun)
  ...
  if sun then
    ...
    system.bodies[1] = Star.n(Vec2D.null(), central_body_mass, math.random(1.2 * (max_radius - min_radius),
      1.2 * max_radius))
    ...
    min_radius = 20
    max_radius = 60
    system.player[1] = Body.create("P1N1", Vec2D.null(), Vec2D.null(), math.random(max_radius / 2, max_radius),
      player_color, central_body_mass, 1)
...
function Game.new(player_name, N_player, N_ai, Player_center, sun)
  ...
  g.quad         = love.graphics.newQuad(-8 * PBGame.screen.w, -8 * PBGame.screen.h, 16 * PBGame.screen.w,
    16 * PBGame.screen.h, g.bg:getWidth(), g.bg:getHeight())
...
function Game:draw()
  ...
  if self.player.selected ~= nil then
    love.graphics.draw(self.bg, self.quad, -PBGame.screen.w * 8, -PBGame.screen.h * 8, 0, 1, 1,
      self.scale * self.player.selected.position.x, self.scale * self.player.selected.position.y)
  else
    love.graphics.draw(self.bg, self.quad, -PBGame.screen.w * 8, -PBGame.screen.h * 8)
  end
  local bfh = PBGame.bigFont:getHeight()
  local sfh = PBGame.smallFont:getHeight()
  ...
  love.graphics.setFont(PBGame.smallFont)
  love.graphics.print(string, 2, bfh + 4 + sfh)
  love.graphics.print('Score : ' .. self.player.score, 2, bfh + 8 + 2 * sfh)
  if self.player.selected ~= nil then
    self:draw_rockets(self.player.selected.rockets, PBGame.smallFont:getWidth(string) + 10, bfh + 2 + 2 * sfh, 4, sfh)
  end
  love.graphics.push()
  love.graphics.translate(PBGame.screen.cx, PBGame.screen.cy)
  ...
  if self.gameover then
    ...
    local w = PBGame.bigFont:getWidth(title)
    love.graphics.print(title, PBGame.screen.cx - w / 2, PBGame.screen.cy - bfh / 2)
    string = "The Winner is: " .. self.winner .. "!"
    w = PBGame.smallFont:getWidth(string)
    love.graphics.setFont(PBGame.smallFont)
    love.graphics.print(string, PBGame.screen.cx - w / 2, PBGame.screen.cy + bfh / 2 + sfh / 2)
  elseif self.in_pause then
    ...
    love.graphics.setFont(PBGame.bigFont)
    local string = "Pause"
    local w = PBGame.bigFont:getWidth(string)
    love.graphics.print(string, PBGame.screen.cx - w / 2, PBGame.screen.cy - bfh / 2)
    string = "Press ESC to resume"
    w = PBGame.smallFont:getWidth(string)
    love.graphics.setFont(PBGame.smallFont)
    love.graphics.print(string, PBGame.screen.cx - w / 2, PBGame.screen.cy + bfh / 2 + sfh)
    string = "Press Q to exit"
    w = PBGame.smallFont:getWidth(string)
    love.graphics.print(string, PBGame.screen.cx - w / 2, PBGame.screen.cy + bfh + sfh * 2)
  end
end
...
function Game:mousepressed(x, y, button)
  ...
  if button == 1 then
    local cx = (x - PBGame.screen.cx) / self.scale
    local cy = (y - PBGame.screen.cy) / self.scale
    if self.player.selected ~= nil then
    ...
    if self.player.selected ~= nil and self.player.selected.rockets > 0 then
      ...
      self.player.launching.x = (x - PBGame.screen.cx)
      self.player.launching.y = (y - PBGame.screen.cy)
    end
...
function Game:mousereleased(x, y, button)
  ...
  if button == 1 and self.player.launching.status and self.player.selected ~= nil then
    self.player.launching.status = false
    local r = self.player.selected:launch((x - PBGame.screen.cx) / self.scale, (y - PBGame.screen.cy) / self.scale,
      self.scale)
    if r ~= nil then
      Universe.bodies[#Universe.bodies + 1] = r
    end
