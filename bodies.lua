...
function Body:draw_launch(x, y, scale)
  ...
  if d > self.s_radius then
    ...
    if d > MaxRocektSpeed ^ 2 then
      local x1 = MaxRocektSpeed * math.cos(a)
      local y1 = MaxRocektSpeed * math.sin(a)
    ...
    local m = s:mod()
    if m > MaxRocektSpeed then
      s = s * MaxRocektSpeed * 1.5 / m
    end
    local it = Universe:preview({ position = Vec2D.n(x0 / scale, y0 / scale) + self.position, mass = 1, speed = s +
    self.speed }, self)
    for i = 1, #it, 4 do
...
function Body:draw(scale)
  ...
  love.graphics.circle("line", self.speed.x, self.speed.y, 4, 4)
...
function Body:impact(obj)
  ...
  if scale == 1 then
    ...
    for i = 1, d do
      to_generate[i] = Debris.n(pos + vbase * 0.07 + Vec2D.rand(10), self.speed * 0.8 + vbase * 0.2 + Vec2D.rand(5),
        self.color, mass)
    end
  end
  if type == Rocket then
...
function Rocket:contains(_)
  return false
end

function Rocket:impact(_)
  self.to_remove = true
  return {}
end
...
function DeadPlanet:itinerary(_)
end
...
function Debris:target(_)
end

function Debris:impact(_)
  self.to_remove = true
  return {}
end
